...
/* harmony import */ var _modules_menucards__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./modules/menucards */ "./src/js/modules/menucards.js");
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  const timerId = setTimeout(() => {
    Object(_modules_modal__WEBPACK_IMPORTED_MODULE_6__["showModal"])(modalWindow);
  }, 50000);
  Object(_modules_tabs__WEBPACK_IMPORTED_MODULE_0__["default"])('.tabcontent', '.tabheader__items', '.tabheader__item', 'tabheader__item_active');
  Object(_modules_modal__WEBPACK_IMPORTED_MODULE_6__["default"])('[data-modal]', '.modal', '.modal__close', timerId);
  Object(_modules_timer__WEBPACK_IMPORTED_MODULE_1__["default"])('2021-04-17', '.timer');
  Object(_modules_menucards__WEBPACK_IMPORTED_MODULE_2__["default"])();
  ...
  Object(_modules_calc__WEBPACK_IMPORTED_MODULE_4__["default"])('.calculating__result span');
  Object(_modules_forms__WEBPACK_IMPORTED_MODULE_5__["default"])('form', '.modal');
});
...
function calc(resultSelector) {
  ...
  let height, weight, age, sex, activity; //set default values for choose boxes from local storage
  ...
  calcResult(); //function to initialize active classes on calc on page start,take values from local storage
  ...
  initLocalSettings('.calculating__choose_big', 'calculating__choose-item_active'); //get result based on formula
  ...
  function getCalcInfo(parentSelector, activeClass) {
    ...
    const elements = document.querySelectorAll(`${parentSelector} div`); //set event listeners for each element
    ...
    elements.forEach(elem => {
      elem.addEventListener('click', e => {
        ...
        console.log(activity, sex); //add active class on clicked element
  ...
  function getInputInfo(selector) {
    const inputs = document.querySelectorAll(`${selector} input`); //set event listeners on input and write values into variables 
  ...
  getCalcInfo('.calculating__choose_big', 'calculating__choose-item_active'); //call func with inputs
...
function forms(form, modalWindow, modalTimerId) {
  ...
  function bindPostData(form) {
    form.addEventListener('submit', e => {
      ...
      form.insertAdjacentElement('afterend', spinner); //take form info
      ...
      const object = JSON.stringify(Object.fromEntries(formData.entries())); //show thanks modal if success or error message on error
      //and reset form + remove spinner

      Object(_services_services__WEBPACK_IMPORTED_MODULE_3__["postData"])('http://localhost:3000/requests', object).then(data => {
        console.log(data);
  ...
  function showThanksModal(msg) {
    ...
    Object(_modal__WEBPACK_IMPORTED_MODULE_2__["showModal"])(modal);
    ...
    setTimeout(() => {
      ...
      Object(_modal__WEBPACK_IMPORTED_MODULE_2__["closeModalHelp"])(modal, modalTimerId);
    }, 4000);
  }
}
...
/*!*********************************!*\
  !*** ./src/js/modules/modal.js ***!
  \*********************************/
/*! exports provided: default, showModal, closeModalHelp, showModalByScroll */
/***/ (function(module, __webpack_exports__, __webpack_require__) {
...
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "showModalByScroll", function() { return showModalByScroll; });
...
function modal(triggerSelector, modalSelector, closeTriggerSelector, modalTimerId) {
  const modal = document.querySelector(modalSelector),
        ...
        modalClose = document.querySelector(closeTriggerSelector); //open modal window on trigger
...
function slider({
  ...
  let slideIndex = 1; //Перемення для отступа

  let offset = 0; //внутренняя обертка будет занимать большое пространство и передвигать слайды, добавляем ей свойства  

  sliderInner.style.cssText = `display: flex; transition: 0.5s all; width: ${100 * slides.length}%`; //Скрываем элементы у враппера слайдов,которые не попадают в область видимости

  sliderWrapper.style.overflow = "hidden"; //слайды должны быть фиксированной и одинаковой ширины,чтобы влезать
  ...
  slider.append(dots); //create dots , set Attributes, classes, and append to parent and push to dotsArr so we can easy manipulate them
  ...
  dotsArr.forEach(dot => {
    dot.addEventListener('click', e => {
      ...
      slideIndex = slideTo; //Устанавливаем отступ, двигаем слайдер в зависимости от полученного аттрибута

      offset = +sliderWidth.slice(0, sliderWidth.length - 2) * (slideTo - 1);
      sliderInner.style.transform = `translateX(-${offset}px)`; //set styles for active dot

      dotsArr.forEach(dot => {
        dot.style.opacity = '.5';
      });
      dotsArr[slideIndex - 1].style.opacity = '1'; //set current slide counter
...
const getData = async url => {
  const response = await fetch(url); //if error throw error
